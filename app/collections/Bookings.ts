...
const Bookings: CollectionConfig = {
  ...
  hooks: {
    ...
    afterChange: [
      ...
      // Driver notification email on assignment
      async ({ doc, previousDoc, req, operation }) => {
        if (operation !== 'update' && operation !== 'create') return doc

        // Check if booking is confirmed and has a driver assigned
        if (doc.status !== 'confirmed' || !doc.driver) return doc

        // Get driver ID from doc (can be object or number)
        const docDriverId = typeof doc.driver === 'object' ? doc.driver?.id : doc.driver
        if (!docDriverId) return doc

        // Check if this is a new confirmation or driver changed
        const prevDriverId = previousDoc?.driver
          ? (typeof previousDoc.driver === 'object' ? previousDoc.driver?.id : previousDoc.driver)
          : null
        const wasConfirmed = previousDoc?.status === 'confirmed'
        const driverChanged = prevDriverId !== docDriverId
